  ...
  grunt.registerTask('generate-font', function() {
    ...
    webfontsGenerator({
      ...
      rename: function(path) {
        // We just want the filename, so split on backslashes and pop the last one.
        let fileName = path.split('/').pop();

        let iconName = _.result(_.find(icons, function(icon) {
          // We need to do the same as above so we can compare apples to apples
          let svgName = icon.svg.split('/').pop();
